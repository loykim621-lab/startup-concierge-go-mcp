  ...
  server.registerTool(
    "locate_form_source",
    {
      ...
      description:
        ...
        "①서식 내용을 전체 복사해 채팅에 붙여넣기(권장 — 파일 첨부가 안 되는 채팅에서도 동작) " +
        "②파일 첨부를 지원하는 앱에서는 PDF로 변환해 올리기 를 사용자에게 안내하세요. " +
        "완성본은 DOCX 파일로 제공됩니다.",
    ...
    async (args): Promise<CallToolResult> => {
      ...
      const lines: string[] = [
        ...
        "",
        "HWP 파일은 AI가 직접 읽기 어렵습니다. 아래 방법으로 서식을 전달해 주세요:",
        "  ① (권장) 한글 또는 무료 한컴오피스 뷰어에서 서식을 열어 전체 선택(Ctrl+A)·복사(Ctrl+C)한 뒤",
        "     이 채팅에 그대로 붙여넣어 주세요 — 파일 첨부 없이 지금 이 채팅에서 바로 됩니다.",
        "  ② 파일 첨부를 지원하는 AI 앱을 쓰신다면, PDF로 변환해 올리셔도 잘 인식됩니다.",
      ...
      return textResult(lines.join("\n"), {
        ...
        주관기관: g.주관기관,
        원문URL: g.원문URL,
        안내: "파일을 대신 받아오지 않습니다. HWP는 ①내용 전체복사→채팅 붙여넣기(권장·첨부 불필요) ②첨부 지원 앱에서는 PDF 변환 업로드. 완성본은 DOCX로 제공.",
        출처: g.source,
        수집시점: g.collected_at?.slice(0, 10) ?? null,
      });
    }
