...
5-1) 서류 원스톱: locate_form_source로 서식 출처 안내 → 서식이 HWP 파일이면 "AI가 읽기 어려우니 ①PDF로 변환해 올리기(가장 잘 인식) ②내용 붙여넣기"를 안내하고 완성본은 DOCX로 제공된다고 알린다 → 사용자가 서식(PDF 업로드분은 호스트가 읽어 텍스트로)을 주면 analyze_form → required_inputs 교차질문 → compose_application → plan_review 재검증 → export_document(다운로드+전문)까지 이어서 안내한다.
